...
@section('content')
    <form id="form-data"
        action="{{ isset($data) ? route('dashboard.agents.update', $data) : route('dashboard.agents.store') }}"
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    <div class="row">
                        ...
                        <x-inputs.text label="{{ __('dash.email') }}" name="email" required=""
                            data="{{ isset($data) ? $data->email : '' }}" />

                        <x-inputs.text label="{{ __('dash.phone') }}" name="phone_number" required=""
                            data="{{ isset($data) ? $data->phone_number : '' }}" />

                        <x-inputs.text label="{{ __('dash.id_number') }}" name="id_number" required=""
                            data="{{ isset($data) ? $data->id_number : '' }}" />

                        <x-inputs.text label="{{ __('dash.company') }}" name="company" required=""
                            data="{{ isset($data) ? $data->company : '' }}" />

                        <x-inputs.text label="{{ __('dash.job_title') }}" name="job_title" required=""
                            data="{{ isset($data) ? $data->job_title : '' }}" />

                        <x-inputs.password label="{{ __('dash.password') }}" name="password" required="" />

                        <x-inputs.password label="{{ __('dash.password_confirmation') }}" name="password_confirmation"
                            required="" />
                    </div>
                </div>
            </div>
